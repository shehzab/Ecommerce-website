...
const UserList = () => {
  ...
  const [ediatableUserEmail, setEditableUserEMail] = useState('')
              ...
              {users.map(user=>(
                <tr key={user._id}>
                  ...
                  <td className="px-4 py-2">
                    {editableUserId === user._id ? (
                        <div className="flex items-center">
                          <input type="text" value={ediatableUserEmail} onChange={e=>setEditableUserEMail(e.target.value)} className="w-full p-2 border rounded-lg" />
                          <button onClick={()=> updateHandler(user._id)} className="ml-2 bg-blue-500 text-white py-2 px-4 rounded-lg ">
                            <FaCheck />
                            </button>
                        </div>
